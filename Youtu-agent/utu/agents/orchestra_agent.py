...
class OrchestraAgent(BaseAgent):
    ...
    async def cleanup(self):
        ...
        # PlannerAgent and SimpleWorkerAgent don't have cleanup methods
        # They rely on garbage collection and underlying system cleanup
        pass
    ...
    async def run(self, input: str, trace_id: str = None) -> OrchestraTaskRecorder:
        ...
        with trace(workflow_name="orchestra_agent", trace_id=trace_id):
            ...
            result = await self.synthesize(task_recorder)
    ...
    async def synthesize(self, task_recorder: OrchestraTaskRecorder) -> AnalysisResult:
        """Step3: Synthesize results from all worker agents"""
        with function_span("synthesis") as span_fn:
            ...
            if task_recorder.plan and task_recorder.plan.analysis:
                synthesis_parts.append(f"## Analysis\n{task_recorder.plan.analysis}\n")

            # Add results from each worker agent
            for i, (result, subtask) in enumerate(zip(task_recorder.task_records, task_recorder.plan.todo), 1):
                synthesis_parts.append(f"## Agent {i}: {subtask.agent_name}")
                synthesis_parts.append(f"**Task:** {subtask.task}")
                synthesis_parts.append(f"**Result:**\n{result.output}\n")

            # Create final synthesis
            final_output = "\n".join(synthesis_parts)

            analysis_result = AnalysisResult(output=final_output)
            task_recorder.add_reporter_result(analysis_result)  # Reuse existing method for consistency
